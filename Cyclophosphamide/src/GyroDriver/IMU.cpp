	...
	while (!stop) {
		...
		if (bytes_read > 0) {
			...
			while (i < bytes_read) {
				...
				int packet_length = imu->DecodePacketHandler(
						&protocol_buffer[i], bytes_remaining);
				if (packet_length > 0) {
					packets_received++;
					update_count++;
					i += packet_length;
					...
					packet_length = IMUProtocol::decodeStreamResponse(
							&protocol_buffer[i], bytes_remaining, stream_type,
							gyro_fsr_dps, accel_fsr, update_rate_hz,
							yaw_offset_degrees, q1_offset, q2_offset, q3_offset,
							q4_offset, flags);
					if (packet_length > 0) {
						packets_received++;
						imu->SetStreamResponse(stream_type, gyro_fsr_dps,
								accel_fsr, update_rate_hz, yaw_offset_degrees,
			...
			if (!stream_response_received
				...
				cmd_packet_length = IMUProtocol::encodeStreamCommand(
						protocol_buffer, imu->current_stream_type,
						imu->update_rate_hz);
				last_stream_command_sent_timestamp = Timer::GetFPGATimestamp();
				pport->Write(protocol_buffer, cmd_packet_length);
				pport->Flush();
...
int IMU::DecodePacketHandler(char *received_data, int bytes_remaining) {
	...
	int packet_length = IMUProtocol::decodeYPRUpdate(received_data,
			bytes_remaining, yaw, pitch, roll, compass_heading);
	if (packet_length > 0) {
...
void IMU::InternalInit(SerialPort *pport, uint8_t update_rate_hz,
		char stream_type) {
...
void IMU::InitIMU() {
	...
	int packet_length = IMUProtocol::encodeStreamCommand(protocol_buffer,
			current_stream_type, update_rate_hz);
...
void IMU::SetStreamResponse(char stream_type, uint16_t gyro_fsr_dps,
		uint16_t accel_fsr, uint16_t update_rate_hz, float yaw_offset_degrees,
